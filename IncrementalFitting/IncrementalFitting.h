...
struct IncrementalFitting : public Classifier<IncrementalFitting<Function, Optimizer>>
{
    ...
    void fit_ (const Mat& X, const Veci& y, int K = 5, double minGap = 1e-5)
    {
        ...
        for(int k = 0; k < K; ++k)
        {
            ...
            Vec w = Vec::NullaryExpr(N+3, [&](int){ return randDouble(0.1, 1.0); });
            ...
            predictions = predictions.array() + weights(k) *
