#ifndef ML_MULTINOMIAL_A_POSTERIORI_DISTRIBUTION_H
#define ML_MULTINOMIAL_A_POSTERIORI_DISTRIBUTION_H

#include "../../Modelo.h"
#include "../Multinomial/Multinomial.h"


struct MultinomialPost : public Multinomial
{
	MultinomialPost (int K) : MultinomialPost(Vec::Constant(K, 1.0))
	{
	}

	template <class V = Vec>
	MultinomialPost (V&& alpha_ = V::Constant(1, 1.0))
	{
	...
	void fit (const Veci& x, int K_)
	{
		...
		mu = (mu.array() + alpha.array() - 1.0) / (alpha0 + x.rows() - K);
	}
...
#endif // ML_MULTINOMIAL_A_POSTERIORI_DISTRIBUTION_H