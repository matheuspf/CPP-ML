...
int main ()
{
    ...
    auto [X_train, y_train, X_test, y_test] = trainTestSplit(X, y, 0.5, 42);
    ...
    FOR(i, y_test.rows())
        y_pred(i) = dlr.predict(Vec(X_test.row(i)));
    ...
    //     dlr.learn(X_train, y_train, x, z);

    //     Vec y_pred(y_test.rows());
        
    //     FOR(i, y_test.rows())
    //         y_pred(i) = dlr(Vec(X_test.row(i)));
